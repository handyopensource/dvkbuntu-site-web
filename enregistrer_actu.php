    ...
    { 
        if(isset($_POST['enregistrer']))
        {
            if(isset($_SESSION['titre']) AND !empty($_SESSION['titre']) AND isset($_SESSION['texte']) AND !empty($_SESSION['texte']))
            {
                ...
                {
                    $reqactu = $bdd->prepare('SELECT * FROM actualites WHERE titre = ?');
                    $reqactu->execute(array($titre));
                    $actuexist = $reqactu->rowCount();

                    if($actuexist == 1)
                    {
                        $erreur = "Il y a déjà une actualité qui porte ce titre !";
                    }
                    else
                    {
                        ...
                        {  
                            ...
                            if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
                            {
                                ...
                                {                 
                                    $req = $bdd->prepare('INSERT INTO actualites(auteur, titre, description, image) VALUES(:auteur, :titre, :description, :image)');
                                    $req->execute(array(
                                        'auteur' => $_SESSION['nom'],
                                        'titre' => $titre,
                                        'description' => $texte,
                        ...
                        {
                            $req = $bdd->prepare('INSERT INTO actualites(auteur, titre, description) VALUES(:auteur, :titre, :description)');
                                                $req->execute(array(
                                                        'auteur' => $_SESSION['nom'],
                                                        'titre' => $titre,
                                                        'description' => $texte
						...
						<div class="inner">
							...
							<!-- Header -->
								<header id="header">
									...
									<ul class="icons">
                                       <li><a href="mode.php?id=9" style="color: yellow; background-color: black;">Mode malvoyant : </a>&nbsp;<a href="mode.php" style="color:#000000" alt="Changer les contrastes"><i class="fas fa-low-vision fa-lg" id="icone_oeil"></i></a></li>

...
if(isset($_POST['envoi']))
{
    ...
    if(!empty($titre) AND !empty($description))
    {
        ...
        if($titrelenght <= 255 AND $descriptionlenght <= 3000)
        {
            ...
            {  
                ...
                if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
                {
                    ...
                    {                 
                        $req = $bdd->prepare('UPDATE actualites SET titre = :titre, description = :description, image = :image WHERE id = :id');
                        $req->execute(array(
                                'titre' => $titre,
            ...
            {
                $req = $bdd->prepare('UPDATE actualites SET titre = :titre, description = :description WHERE id = :id');
                $req->execute(array(
                        'titre' => $titre,
                        'description' => $description,
                        'id' => $getid
						...
						<div class="inner">
							...
							<!-- Header -->
								<header id="header">
									...
									<ul class="icons">
                                       <li><a href="mode.php?id=15" style="color: yellow; background-color: black;">Mode malvoyant : </a>&nbsp;<a href="mode.php?id=15" style="color:#000000" alt="Changer les contrastes"><i class="fas fa-low-vision fa-lg" id="icone_oeil"></i></a></li>
